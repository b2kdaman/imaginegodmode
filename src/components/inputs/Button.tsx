...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyles = 'rounded-full transition-all duration-300 flex items-center justify-center relative overflow-hidden bg-theme-bg-medium/[0.67] text-theme-text-secondary border border-theme-border backdrop-blur-md';
  const variantStyles = {
    default: 'px-3 py-2 text-xs disabled:opacity-30',
    icon: 'w-9 h-9 disabled:opacity-30',
  };

  // Default icon color is theme text secondary (CSS variable)
  const finalIconColor = iconColor || 'var(--color-text-secondary)';
